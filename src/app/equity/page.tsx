...
import { Sparkles, Target, Users, TrendingUp, Globe } from 'lucide-react';
  ...
  return (
    <main className="relative min-h-screen w-full bg-[#050508] text-white selection:bg-primary/30">
      ...
      <nav className="fixed top-0 left-0 w-full p-8 z-50 flex justify-between items-center">
        <Link href="/" className="group flex items-center gap-4 text-[11px] uppercase tracking-[0.4em] text-white/40 hover:text-white transition-all">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="group-hover:-translate-x-1.5 transition-transform duration-500">
            <path d="M19 12H5M5 12L11 18M5 12L11 6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          Back to Constellation
        </Link>
        <div className="text-[10px] uppercase tracking-[0.8em] text-white/20 font-medium hidden md:block">
      ...
      <div className="relative z-10 pt-32 pb-32 px-6 max-w-6xl mx-auto">
        <header className="mb-24 space-y-10">
          <motion.div
            ...
            className="space-y-3"
          >
            <p className="text-[11px] md:text-xs uppercase tracking-[1em] text-primary/60 font-bold italic">
              Manifesting the Future
            </p>
            <h1 className="text-5xl md:text-7xl font-bold tracking-tighter uppercase leading-none italic">
              The Equity <br />
          ...
          <motion.p
            ...
            className="max-w-xl text-base md:text-lg text-white/70 font-light leading-relaxed tracking-wide italic"
          >
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-24">
          ...
          >
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-white/40 uppercase tracking-widest">Global tech workforce percentage (%)</p>
            </div>
        ...
        <section className="mb-24">
          <Tabs defaultValue="legacy" className="w-full">
            <TabsList className="bg-transparent border-b border-white/10 w-full justify-start gap-10 rounded-none h-auto pb-4 overflow-x-auto no-scrollbar">
              <TabsTrigger value="legacy" className="data-[state=active]:bg-transparent data-[state=active]:text-primary border-none p-0 text-[11px] uppercase tracking-[0.3em] font-bold text-white/30">The Historical Root</TabsTrigger>
              <TabsTrigger value="barriers" className="data-[state=active]:bg-transparent data-[state=active]:text-violet-400 border-none p-0 text-[11px] uppercase tracking-[0.3em] font-bold text-white/30">Structural Barriers</TabsTrigger>
              <TabsTrigger value="future" className="data-[state=active]:bg-transparent data-[state=active]:text-rose-400 border-none p-0 text-[11px] uppercase tracking-[0.3em] font-bold text-white/30">A New Architecture</TabsTrigger>
            </TabsList>
        ...
        <footer className="pt-24 border-t border-white/5 flex flex-col items-center text-center space-y-12">
           <Sparkles className="w-8 h-8 text-primary animate-pulse" />
           <div className="space-y-5">
             <h2 className="text-4xl md:text-6xl font-bold uppercase italic tracking-tighter">Your Code, Your Legacy.</h2>
             <p className="text-[10px] uppercase tracking-[0.8em] text-white/30">Write a future where every star can shine.</p>
           </div>
           <Link href="/" className="px-12 py-4 rounded-full border border-white/10 hover:border-primary/50 hover:bg-primary/10 transition-all text-[11px] uppercase tracking-[0.4em] font-bold">
...
function InsightCard({ icon, title, label, description, delay }: { icon: React.ReactNode, title: string, label: string, description: string, delay: number }) {
  return (
    <motion.div
      ...
      className="glass-morphism rounded-3xl p-7 flex flex-col justify-between"
    >
      ...
      <p className="text-[11px] text-white/50 leading-relaxed mt-4 font-light italic">
        {description}
      </p>
    </motion.div>
  );
